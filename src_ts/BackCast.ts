...
EventsSDK.on("PrepareUnitOrders", order => {
	if (!State.value || (StateMiltiUnit.selected_id !== 1 && order.Issuers[0] !== LocalPlayer!.Hero) || order.OrderType === dotaunitorder_t.DOTA_UNIT_ORDER_TRAIN_ABILITY)
		return true
	const abil = order.Ability as Ability
